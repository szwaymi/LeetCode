...
void rSearch(unsigned int iLocation, unsigned int iStep){
    if(iStep >= gmMeta.iSteps){
        return;
    }
    ...
    for(iConSteps = gmMeta.piSeries[iLocation] ; iConSteps >= 1 ; iConSteps--){
        rSearch(iLocation + iConSteps, iStep + 1);
    }
}

int jump(int* piSeries, int iSize){
    gmMeta.iSteps = 0xFFFFFFFF;
    gmMeta.iGoal = iSize - 1;
    gmMeta.piSeries = piSeries;
    rSearch(0, 0);
    return gmMeta.iSteps;
}
int main(void){
    //Test Data
    //  Macro
